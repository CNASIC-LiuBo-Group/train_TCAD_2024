...
class convAppx_app_lut(torch.autograd.Function):

    @staticmethod
    def forward(ctx, X, weight, bias, mul, padding=1, stride=1, weight_scale=1.0, activation_scale=1.0):
        ...
        X_pad = torch.nn.functional.pad(X, (padding[0], padding[0], padding[0], padding[0]))
        ...
        if X.is_cuda:
            ...
            inp_unf = torch.nn.functional.unfold(X_pad, (f, f), padding=0, stride=stride)
            ...
            out_unf = cuda_app_layers.conv_app_forward(inp_unf_flat, weight_flat, bias,
                                               inp_unf.size(1), inp_unf.size(2),
                                               weight.size(1), b, weight_scale, activation_scale, mul)
