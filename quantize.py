...
import numpy as np
from op import *
from Myfunc import newconv2d
from Myfunc import newliner
from Myfunc import NewConv2d
from Myfunc import NewLiner
...
# ********************* 量化卷积（同时量化A/W，并做卷积） *********************
def quant_activation_only(x, flag):
    activation_quantizer = SymmetricQuantizer(bits=8, observer=MovingAverageMinMaxObserver(
        q_level='L', out_channels=None, bbit=8), activation_weight_flag=1, qaft=False, flag=flag)
    x, scale = activation_quantizer(x)
    return x
...
class QuantConv2d(nn.Conv2d):
        ...
        if not ptq:
            if q_type == 0:
                self.activation_quantizer = SymmetricQuantizer(bits=a_bits, observer=MovingAverageMinMaxObserver(
                    q_level='L', out_channels=None, bbit=a_bits), activation_weight_flag=1, qaft=qaft, flag=flag)
            ...
            self.activation_quantizer = SymmetricQuantizer(bits=a_bits, observer=HistogramObserver(
                q_level='L', percentile=percentile), activation_weight_flag=1, qaft=qaft, flag=flag)
            if weight_observer == 0:
                if q_level == 0:
                    self.weight_quantizer = SymmetricQuantizer(bits=w_bits, observer=MinMaxObserver(
                        q_level='C', out_channels=out_channels), activation_weight_flag=0, qaft=qaft, flag=flag)
                else:
                    self.weight_quantizer = SymmetricQuantizer(bits=w_bits, observer=MinMaxObserver(
                        q_level='L', out_channels=None), activation_weight_flag=0, qaft=qaft, flag=flag)
            else:
                if q_level == 0:
                    self.weight_quantizer = SymmetricQuantizer(bits=w_bits, observer=MovingAverageMinMaxObserver(
                        q_level='C', out_channels=out_channels), activation_weight_flag=0, qaft=qaft, flag=flag)
    ...
    def forward(self, input):
        ...
        if self.flag:
            # output = newconv2d(quant_input, quant_weight, self.bias, self.stride, self.padding, quant_input_scale, quant_weight_scale, self.mul_lut)
            output = NewConv2d.apply(quant_input, quant_weight, self.bias, self.stride, self.padding, quant_input_scale,
                                     quant_weight_scale, self.mul_lut)
        else:
            output = F.conv2d(quant_input, quant_weight, self.bias, self.stride, self.padding, self.dilation,
                              self.groups)
        return output
...
class QuantBNFuseConv2d(QuantConv2d):
    def __init__(self,
                 ...
                 percentile=0.9999,
                 bn_fuse_calib=False):
        super(QuantBNFuseConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation,
                                                groups,
                                                bias, padding_mode)
        self.num_flag = 0
        ...
        if not self.qaft:
            ...
            if self.training:
                ...
                if not self.bn_fuse_calib:
                    weight_fused = self.weight * reshape_to_weight(
                        self.gamma / torch.sqrt(batch_var + self.eps))  # w融batch
                # bn融合校准
                else:
                    weight_fused = self.weight * reshape_to_weight(
            ...
            # 测试态
                ...
                if self.bias is not None:
                    ...
                    bias_fused = reshape_to_bias(self.beta - self.running_mean * (
...
class QuantLinear(nn.Linear):
    def __init__(self,
        ...
        if not ptq:
            ...
            if weight_observer == 0:
                if q_level == 0:
                    self.weight_quantizer = SymmetricQuantizer(bits=w_bits, observer=MinMaxObserver(
                        q_level='FC', out_channels=out_features), activation_weight_flag=0, qaft=qaft, flag=flag)
                else:
                    self.weight_quantizer = SymmetricQuantizer(bits=w_bits, observer=MinMaxObserver(
                        q_level='L', out_channels=None), activation_weight_flag=0, qaft=qaft, flag=flag)
            else:
    ...
    def forward(self, input):
        ...
        if self.flag:
            ...
            output = NewLiner.apply(quant_input, quant_weight, self.bias, quant_input_scale, quant_weight_scale,
                                    self.mul_lut)
        else:
            output = F.linear(quant_input, quant_weight, self.bias)
        # output = F.linear(quant_input, quant_weight, self.bias)
...
class QuantAvgPool2d(nn.AvgPool2d):
    def __init__(self, kernel_size, stride=None, padding=0, ceil_mode=False,
                 count_include_pad=True, divisor_override=None, a_bits=8, q_type=0, qaft=False, ptq=False,
                 percentile=0.9999):
